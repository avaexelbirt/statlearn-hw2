...
library(ggplot2)
ggplot(testData, aes(x = budget, y = popularity, color = lda_predicted_success)) +
...
```{r}
dim(x)  # Check the number of rows and columns in x
length(y)
```
...
```{r}
any(is.na(testData))
length(nn_predictions)
length(testData$success)
