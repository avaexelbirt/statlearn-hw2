...
#write.csv(horror, "horror_movies.csv", row.names = FALSE)
```
...
```
You can see for popularity, there is a non linear relationship positive relationship between popularity and the SHAP values. Initially, popularity positively contributes to the prediction of success, but after reaching a peak SHAP value, the contribution decreases and eventually becomes negative. This suggests that higher levels of popularity initially increase the likelihood of success, but beyond a certain point, it negatively affects the prediction. The maximum SHAP value is 0.5 indicating that the strongest contribution of popularity to the model's prediction is around 50%. Because it is nonlinear is suggests that there is a non linear relationship between popularity and the predictions. Since there is not much variance in the SHAP values, it does not suggest a strong interaction effect with other variables. 
For release year, there is almost a linear relationship between release year and the predictions with release year negatively impacting predictions. The wider spread of SHAP values indicates possible interactions with other features. The maximum SHAP value is 0.2 indicating that the strongest contribution of popularity to the model's prediction is around 20%.
For budget, the curve starts out as negative and then flattens out in the positive direction once budget is greater than 0. This indicates that while budget is negative, it negatively affects the prediction, but once budget is positive it positively contributes to the prediction. Since it is non linear, this represents a non linear relationship between the feature and the prediction. A wide spread in the beginning indicates potential interactions with other variables. 
For runtime, there is almost a linear relationship between runtime and the predictions with runtime positively impacting predictions. The wider spread of SHAP values indicates possible interactions with other features. The maximum SHAP value is 0.12 indicating that the strongest contribution of popularity to the model's prediction is around 12%.
...
# Function to train a random forest model and compute AUC
...
# Function to train a logistic regression model and compute accuracy
train_logit_model <- function(predictors, data) {
  model <- glm(success ~ ., data = data[, c(predictors, "success")], family = binomial)
...
# Train random forest models and compute AUC for each predictor set
...
# Train logistic regression models and compute accuracy for each predictor set
logit_accuracy_1 <- train_logit_model(predictor_set_1, df_for_class_lg)
logit_accuracy_2 <- train_logit_model(predictor_set_2, df_for_class_lg)
logit_accuracy_3 <- train_logit_model(predictor_set_3, df_for_class_lg)
logit_accuracy_4 <- train_logit_model(predictor_set_4, df_for_class_lg)

# Create a performance comparison table
performance_comparison <- data.frame(
  Model = c("Random Forest (Set 1)", "Random Forest (Set 2)", "Random Forest (Set 3)", "Random Forest (Set 4)",
            "Logistic Regression (Set 1)", "Logistic Regression (Set 2)", "Logistic Regression (Set 3)", "Logistic Regression (Set 4)"),
  AUC_or_Accuracy = c(rf_auc_1, rf_auc_2, rf_auc_3, rf_auc_4,
                      logit_accuracy_1, logit_accuracy_2, logit_accuracy_3, logit_accuracy_4)
)
...
```{r}
compute_aic_bic <- function(predictors, data) {
  model <- glm(success ~ ., data = data[, c(predictors, "success")], family = binomial)
  aic <- AIC(model)
  bic <- BIC(model)
...
aic_bic_set_1 <- compute_aic_bic(predictor_set_1, df_for_class_lg)
aic_bic_set_2 <- compute_aic_bic(predictor_set_2, df_for_class_lg)
aic_bic_set_3 <- compute_aic_bic(predictor_set_3, df_for_class_lg)
aic_bic_set_4 <- compute_aic_bic(predictor_set_4, df_for_class_lg)

aic_bic_comparison <- data.frame(
  Predictor_Set = c("Set 1", "Set 2", "Set 3", "Set 4"),
  AIC = c(aic_bic_set_1["AIC"], aic_bic_set_2["AIC"], aic_bic_set_3["AIC"], aic_bic_set_4["AIC"]),
  BIC = c(aic_bic_set_1["BIC"], aic_bic_set_2["BIC"], aic_bic_set_3["BIC"], aic_bic_set_4["BIC"])
)
...
```
Based on AIC and BIC, one should choose Set 2 because it has the lowest AIC and BIC values, indicating the best balance of fit and complexity among the four models.
...
```{r warning=FALSE}
interaction_model <- glm(success ~ budget * release_year + popularity, data = df_for_class_lg, family = binomial)
...
plot(effect_plot)
```
All predictors show a somewhat linear relationship, indicating a constant rate of change in the log odds/probability of success. Budget is pretty constant, whereas release year shows a negative relationship: as the year of release increases the probability of success decreases. Popularity has a positive relationship: as popularity increases the probability of success increases. Budget has the widest confidence interval (shaded region) meaning there is greater uncertainty in the estimated effect than popularity and release year. 
